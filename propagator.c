...
bool init(clingo_propagate_init_t *init, propagator_t *data) {
  ...
  if (!clingo_signature_create("__parity", 2, true, &sig)) { return false; }
  ...
  if (!clingo_signature_create("__parity", 3, true, &sig)) { return false; }
  // get an iterator after the last place/2 atom
  // (atom order corresponds to grounding order (and is unpredictable))
  if (!clingo_symbolic_atoms_end(atoms, &atoms_ie)) { return false; }
  ...
  for (auto it = symbol_to_literal.begin(); it != symbol_to_literal.end(); ++it) {
    ...
    it->second = lit; solver_literal.insert(lit);
  }
...
bool gauss_elimation(clingo_propagate_control_t *control, const clingo_literal_t *changes, size_t size, propagator_t *data) {
  ...
  for (size_t gqhead = 0; gqhead < size; ++gqhead)
  {
    ...
    for (; i != end; i++) {
      ...
      if (data->gmatrixes[i->matrix_num]->find_truths2(
              i, j, p.var(), i->row_id, data->gqueuedata[i->matrix_num])) {
        continue;
      }
      else {
