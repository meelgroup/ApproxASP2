...
unsigned Bounded_counter(clingo_control_t *ctl, configuration *con, bool gen_xor, bool prefix_slice, bool no_xor = false, unsigned m_value = 0) {
    if (!no_xor && gen_xor) {
    ...
    clingo_control_solve(ctl, clingo_solve_mode_async | clingo_solve_mode_yield, NULL, 0, NULL, NULL, &handle);
    bool finished = false;  
...
SATCount LogSATSearch(clingo_control_t* control, configuration* con, int m_prev) {
    ...
    while(num_explored < con->number_of_active_atoms) {
        ...
        if (result == -1) {
            if (repeat_try < 2) {
                generate_xor = true; repeat_try++;
            } 
            else {
                m_value++; repeat_try = 0;
            }
        }
            ...
            if (finder != big_cell.end() && finder->second == false) {
                ...
                ret.hashCount = m_value + 1; return ret;
            }
            big_cell[m_value] = true;
            if (abs(m_value - m_prev) < 2 && m_prev != 0) {
                lo_index = m_value; m_value++;
            } 
            else if ((lo_index + (m_value - lo_index)*2) >= hi_index - 1) {
                lo_index = m_value; m_value = (lo_index + hi_index) >> 1;
            }
            else
            ...
            if (finder != big_cell.end() && finder->second == true) {
                ...
                ret.hashCount = m_value; return ret;
            }
            big_cell[m_value] = true;
            count_list[m_value] = result;
            hi_index = m_value;
            if (abs(m_value - m_prev) < 3 && m_prev != 0) {
                hi_index = m_value; m_value--;
            }
            else {
                if (hash_prev > m_value)
                    hash_prev = 0;
                hi_index = m_value; lo_index = hash_prev; m_value = (lo_index + hi_index) >> 1;
            }
        }
        hash_prev = swap_var;
    }
...
SATCount ApproxSMCCore(clingo_control_t* control, configuration* con) {
    int counter = 0, prev_cells = 1, minHash = 0, medSolCount = 0;   // in previous version prev_cells = 2 ^ 1 = 2
