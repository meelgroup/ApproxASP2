...
bool print_solution(clingo_model_t const *model, model_buffer_t *data)
{
  ...
  if (!print_model(model, data, " ~atoms", clingo_show_type_complement | clingo_show_type_atoms))
  {
    goto error;
  }
  ...
  while (scan < argc)
  {
    ...
    else if (!strcmp(arg, "--input"))
    {
      ...
      if (problem.argu_count == 0) 
        ...
        problem.asp_argument = (const char **) realloc(problem.asp_argument, (problem.argu_count + 1) * sizeof(char *));
      }
