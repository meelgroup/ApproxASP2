...
char* translation(clingo_control_t** ctl, configuration *con, bool debug, std::ostream& debug_file, unsigned start = 1, unsigned end = -1) {
  std::ofstream myfile;
  ...
  if (debug) {
    ...
    condition = ":- { __parity(ID,even,X) } = N, N\\2!=0, __parity(ID,even).  \
